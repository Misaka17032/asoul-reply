        ...
        while True:
            ...
            if mpu.get_model_parallel_rank() == 0:
                if args.input_text:
                    ...
                    raw_text = e_encode(input("\nContext prompt (stop to exit) >>> "))[0:512]
                    while not raw_text:
                        print('Prompt should not be empty!')
                        raw_text = input("\nContext prompt (stop to exit) >>> ")
                raw_text = "对话上文:{}回复:".format(raw_text)
                if "stop" in raw_text:
                    terminate_runs = 1
                else:
            ...
            while counter < (org_context_length + args.out_seq_length):
                ...
                if mpu.get_model_parallel_rank() == 0 and (counter % 16 == 0 or token_end != -1):
                   ...
                   print("\nContext:", e_decode(raw_text)[5:-3], flush=True)
                   trim_decode_tokens = decode_tokens[len(raw_text):decode_tokens.find("<eod>")]
                   print("\nAsoul reply bot:", e_decode(trim_decode_tokens), flush=True)
                if token_end != -1:
                   #print(token_end)
                   break
...
def e_decode(text):
    text = text.replace("\\n", "\n")
    for k in emojis.keys():
        text = text.replace(emojis[k], k)
    if text[0:4] == "回复 @" and ":" in text:
...
if __name__ == "__main__":
    ...
    f = open("emojis.json", "r", encoding="utf-8")
